...
//        }
    ...
    void poll(int timeout) { // timeout in milliseconds
        ...
        while (true) {
            ...
            if (false) { }
				...
				std::cout << "recieve ret <=0  step1 readyState: "<< readyState<< " error="<< GetLastError()<< std::endl;
                readyState = CLOSED;
        ...
        while (txbuf.size()) {
            ...
            if (false) { } // ??
				...
				std::cout << "recieve ret step1 <=0 readyState: " << readyState << std::endl;
                readyState = CLOSED;
                fputs(ret < 0 ? "Connection error!\n" : "Connection closed!\n", stderr);
                if (ret < 0) {
                    std::cout << "Connection error!\n" << std::endl;
        ...
        if (!txbuf.size() && readyState == CLOSING) {
            ...
            readyState = CLOSED;
            std::cout << "readyState  is CLOSEING!"<<" txbuf.size():"<< txbuf.size() << " readyState:"<< readyState << std::endl;
        }
    }
    ...
    virtual void _dispatchBinary(BytesCallback_Imp & callable) {
        ...
        while (true) {
            ...
            if (false) { }
				...
				std::cout << "ws.opcode == wsheader_type::CLOSE" << std::endl;
                close();
            }
            else {
    ...
    void send(const std::string& message) {
		std::string tempStr = UrlUTF8(const_cast<char*>(message.c_str()));
    ...
    void close() {
        ...
        if(readyState == CLOSING || readyState == CLOSED) { return; }
        readyState = CLOSING;
		std::cout << "close() the interface" << std::endl;
